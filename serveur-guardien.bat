...
powershell -NoProfile -ExecutionPolicy Bypass -Command "Get-CimInstance Win32_Process -Filter \"Name='python.exe'\" | Where-Object { \"$($_.CommandLine)\" -match 'serve_all.py' -and (\"$($_.ExecutablePath)\" -notmatch 'C:\\\\Python') -and (\"$($_.CommandLine)\" -notmatch 'travaille-scolaire') } | ForEach-Object { Stop-Process -Id $_.ProcessId -Force -ErrorAction SilentlyContinue; Write-Output ('killed foreign serve_all ' + $_.ProcessId) }" >> lami-server.log 2>&1
